...
import { createPlaybackLicenseId } from "./entitlements.ts";
...
/**
 * get the show associated with a node
 * @param ctx
 * @param feed
 * @returns
 */
export async function getShow(
  ctx: HandlerContextVariables,
  feed: StudioNode,
): Promise<[StudioNode, string]> {
  // this is the feeds parent
  // it could be category = 'FEED' or 'SHOW'
  const parent = await ctx.db.elwood.query.selectFrom(
    "studio_node",
  )
    .selectAll()
  ...
  if (parent.category === "FEED") {
    const grandparent = await ctx.db.elwood.query.selectFrom(
      "studio_node",
    )
      .selectAll()
      .where(
...
export type GetMediaForEpisodeInput = {
  id: string;
  category: "PUBLIC" | "PRIVATE";
};

export type GetMediaForEpisodeResult = {
  audio: StudioNode | undefined;
  audio_playback_license_id: string | undefined;
  video: StudioNode | undefined;
  video_playback_license_id: string | undefined;
};

export async function getMediaForEpisode(
  ctx: HandlerContextVariables,
  input: GetMediaForEpisodeInput,
): Promise<GetMediaForEpisodeResult> {
  let audio_playback_license_id: string | undefined;
  let video_playback_license_id: string | undefined;

  const audio = await ctx.orm.maybeStudioNode((q) => {
    return q.where("parent_id", "=", input.id)
      .where("category", "=", "AUDIO")
      .where("sub_category", "=", input.category);
  });

  const video = await ctx.orm.maybeStudioNode((q) => {
    return q.where("parent_id", "=", input.id)
  ...
  if (audio?.id) {
    audio_playback_license_id = await createPlaybackLicenseId(
      ctx,
      {
        node: audio,
      },
  ...
  if (video?.id) {
    video_playback_license_id = await createPlaybackLicenseId(
      ctx,
      {
        node: video,
      },
